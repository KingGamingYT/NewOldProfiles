...
function FlexInfo(props) {
	...
	const filterCheck = activityCheck({ activities: [activity] });
	return BdApi.React.createElement("div", { className, style }, BdApi.React.createElement(Header, { activity, channel, check }), BdApi.React.createElement(ActivityType, { filterCheck, activity, inventoryEntry, voice, stream, channel, type }));
}
...
function ActivityCard({ user, activity, check }) {
	...
	), BdApi.React.createElement(FlexInfo, { className: "contentImagesProfile content", style: { display: "grid", flex: "1", marginBottom: "3px" }, activity, inventoryEntry, check, type: "PLAYING" }), BdApi.React.createElement("div", { className: "buttonsWrapper actionsProfile" }, BdApi.React.createElement(ActivityButtons, { user, activity, application, onAction: action }))));
}
...
function FriendsSince({ user }) {
	const friendSinceDate = RelationshipStore.getSince(user.id);
	return BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement(SectionHeader, null, locale.Strings.FRIENDS_SINCE()), BdApi.React.createElement(
		"div",
		{
...
function FailedToLoadProfileNotice({}) {
	return BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement(Card, { messageType: "warn", key: "warn" }, locale.Strings.USER_PROFILE_LOAD_ERROR()));
}
...
function AboutTab({ data, user, currentUser, displayProfile }) {
	...
	return BdApi.React.createElement(Scroller, { type: "INFO", padding: 12 }, displayProfile._userProfile?.fetchError && BdApi.React.createElement(FailedToLoadProfileNotice, null), displayProfile?.private && BdApi.React.createElement(PrivateProfileNotice, { username: user.globalName || user.username }), displayProfile?.pronouns && BdApi.React.createElement(PronounsBuilder, { displayProfile }), BdApi.React.createElement(BioBuilder, { displayProfile }), BdApi.React.createElement(RoleBuilder, { user, data, displayProfile }), BdApi.React.createElement(MemberDateBuilder, { data, user }), relationship === 1 && BdApi.React.createElement(FriendsSince, { user }), BdApi.React.createElement(NoteBuilder, { user }), betterdiscord.Data.load("boardTab") && user.id === currentUser.id && BdApi.React.createElement(BoardButtonBuilder, { user }), BdApi.React.createElement(ConnectionCards, { user, connections }), BdApi.React.createElement("div", { "aria-hidden": true, style: { pointerEvents: "none", minHeight: "0px", minWidth: "1px", flex: "0 0 auto", height: "8px" } }));
}
...
class NewOldProfiles {
	...
	async start() {
		...
		betterdiscord.Patcher.after(await betterdiscord.Webpack.waitForModule(betterdiscord.Webpack.Filters.bySource("originGuildId", '"retrying"')), "A", (that, [props], res) => {
			patcher.patch(res.props.children, (props2, res2) => {
				const button = betterdiscord.Utils.findInTree(res2, (tree) => tree && Object.hasOwn(tree, "parentComponent"), { walkable: ["props", "children"] });
