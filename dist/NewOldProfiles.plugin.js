...
function AboutTab({ data, user, currentUser, displayProfile }) {
	...
	return BdApi.React.createElement(Scroller, { type: "INFO", padding: 12 }, displayProfile?.private && BdApi.React.createElement(PrivateProfileNotice, { username: user.globalName || user.username }), displayProfile?.pronouns && BdApi.React.createElement(PronounsBuilder, { displayProfile }), BdApi.React.createElement(BioBuilder, { displayProfile }), BdApi.React.createElement(RoleBuilder, { user, data, displayProfile }), BdApi.React.createElement(MemberDateBuilder, { data, user }), BdApi.React.createElement(NoteBuilder, { user }), betterdiscord.Data.load("boardTab") && user.id === currentUser.id && BdApi.React.createElement(BoardButtonBuilder, { user }), BdApi.React.createElement(ConnectionCards, { user, connections }), BdApi.React.createElement("div", { "aria-hidden": true, style: { pointerEvents: "none", minHeight: "0px", minWidth: "1px", flex: "0 0 auto", height: "8px" } }));
}
