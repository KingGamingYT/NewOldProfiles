...
const [
	...
	{ filter: betterdiscord.Webpack.Filters.bySource("tag", "isCurrentUser", "widgetType", "TAG_REMOVED"), searchDefault: false },
...
function ActivityCard({ user, activity, check }) {
	...
	), BdApi.React.createElement("div", { className: "contentImagesProfile content", style: { display: "grid", flex: "1", marginBottom: "3px" } }, BdApi.React.createElement("div", { className: "nameNormal textRow ellipsis", style: { fontWeight: "600" } }, (check?.listening || check?.watching) && [2, 3].includes(activity?.type) ? activity.details : activity.name), !(filterCheck?.listening || filterCheck?.watching) && BdApi.React.createElement("div", { className: "details textRow ellipsis" }, activity.details), BdApi.React.createElement("div", { className: "state textRow ellipsis" }, activity?.party && activity?.party?.size ? activity.state + " (" + activity.party.size[0] + " of " + activity.party.size[1] + ")" : activity.state), activity?.timestamps?.end ? BdApi.React.createElement("div", { className: "mediaProgressBarContainer" }, BdApi.React.createElement(MediaProgressBar, { start: activity?.timestamps?.start || activity?.created_at, end: activity?.timestamps?.end })) : BdApi.React.createElement(ActivityTimer, { activity })), BdApi.React.createElement("div", { className: "buttonsWrapper actionsProfile" }, BdApi.React.createElement(ActivityButtons, { user, activity }))));
}
...
function HeaderInnerBuilder({ user, currentUser, displayProfile, tagName, displayName }) {
	...
	) : BdApi.React.createElement("div", { className: "displayName" }, displayName || tagName), !betterdiscord.Data.load("disableDiscrim") && displayProfile._userProfile?.legacyUsername ? BdApi.React.createElement(
		"div",
		{
			className: "nameTag",
