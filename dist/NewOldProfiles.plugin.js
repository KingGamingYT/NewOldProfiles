...
const locale = {
	Strings: {
		...
		PLAYING_A_GAME: getIntlString("iKo3yJ"),
...
function DiscordTag({ user, displayProfile, tagName, displayName }) {
	...
	) : BdApi.React.createElement("div", { className: "displayName" }, displayName || tagName), !informationHidden && !betterdiscord.Data.load("disableDiscrim") && legacyName ? BdApi.React.createElement(
		"div",
		{
			className: "nameTag",
...
function ActivityCard({ user, activity, check }) {
	...
	return BdApi.React.createElement("div", { className: "activityProfile activity", id: `${activity.created_at}-${activity.type}`, key: `${activity.created_at}-${activity.type}` }, BdApi.React.createElement(ActivityHeader$1, { activity, check }), BdApi.React.createElement("div", { className: "bodyNormal", style: { display: "flex", alignItems: "center", width: "auto" } }, BdApi.React.createElement(
		...
		activity?.assets && activity?.assets.large_image && !activity?.platform?.includes("xbox") && BdApi.React.createElement(
			RichImageAsset,
			{
				url: activity?.assets?.large_image?.includes("external") ? `https://media.discordapp.net/external/${activity.assets.large_image.substring(activity.assets.large_image.indexOf("/"))}` : `https://cdn.discordapp.com/app-assets/${activity.application_id}/${activity?.assets.large_image}.png`,
		...
		activity?.assets && activity?.assets?.large_image && activity?.assets?.small_image && BdApi.React.createElement(
			RichImageAsset,
			{
				url: activity?.assets?.large_image?.includes("external") ? `https://media.discordapp.net/external/${activity.assets.small_image.substring(activity.assets.small_image.indexOf("/"))}` : `https://cdn.discordapp.com/app-assets/${activity.application_id}/${activity?.assets.small_image}.png`,
...
function CustomCard({ activities }) {
	...
	return _activities.map((activity) => BdApi.React.createElement("div", { className: "activity" }, BdApi.React.createElement(ActivityHeader, { activity }), BdApi.React.createElement(CustomActivityContent, { activity, activities: _activities })));
}
...
function RoleBuilder({ user, data }) {
	...
	return BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement(SectionHeader, null, serverMember?.roles?.length !== 1 ? locale.Strings.ROLES : locale.Strings.ROLE), BdApi.React.createElement(RoleRenderer, { user, currentUser: data.currentUser, guild: GuildStore.getGuild(data?.guildId) }));
}
