...
const [
	...
	{ filter: betterdiscord.Webpack.Filters.byStrings("htmlFor", "asContainer"), searchExports: true },
	...
	{ filter: betterdiscord.Webpack.Filters.bySource("tag", "isCurrentUser", "widgetType", "TAG_REMOVED"), searchExports: true }
);
...
// presence.jsx
const headers = {
	0: intl.intl.formatToPlainString(intl.t["iKo3yM"]),
	1: intl.intl.formatToPlainString(intl.t["4CQq9f"], { name: "" }),
	2: intl.intl.formatToPlainString(intl.t["NF5xoq"], { name: "" }),
	3: intl.intl.formatToPlainString(intl.t["pW3Ip6"], { name: "" }),
	5: intl.intl.formatToPlainString(intl.t["QQ2wVF"], { name: "" })
};
...
function ActivityCard({ user, activity, check }) {
	...
	return BdApi.React.createElement("div", { className: "activityProfile activity", id: activity.created_at + "-" + activity.type, key: activity.created_at + "-" + activity.type }, BdApi.React.createElement("h3", { className: "headerTextNormal headerText", style: { color: "var(--white)", marginBottom: "8px" } }, (check?.listening || check?.watching) && [2, 3].includes(activity?.type) ? headers[activity.type] + activity?.name : filterCheck?.xbox || filterCheck?.playstation ? intl.intl.formatToPlainString(intl.t["A17aMz"], { platform: activity?.platform }) : headers[activity.type]), BdApi.React.createElement("div", { className: "bodyNormal", style: { display: "flex", alignItems: "center", width: "auto" } }, BdApi.React.createElement("div", { className: "assets", style: { position: "relative" } }, activity?.assets && activity?.assets.large_image && BdApi.React.createElement(TooltipBuilder, { note: activity.assets.large_text || activity?.details }, shouldLargeFallback ? BdApi.React.createElement(FallbackAsset, { className: "assetsLargeImage" }) : BdApi.React.createElement(
		"img",
		{
	...
	), !(user.bot || activity?.assets || activity?.application_id || ApplicationStore.getApplication(activity?.application_id)?.icon) && BdApi.React.createElement(FallbackAsset, { style: { width: "40px", height: "40px" } }), activity?.assets && activity?.assets?.large_image && activity?.assets?.small_image && BdApi.React.createElement(TooltipBuilder, { note: activity.assets.small_text || activity?.details }, shouldSmallFallback ? BdApi.React.createElement(FallbackAsset, { className: "assetsSmallImage" }) : BdApi.React.createElement(
		"img",
		{
...
function SpotifyCards({ user, activities }) {
	...
	))), BdApi.React.createElement("div", { className: "contentImagesProfile content" }, BdApi.React.createElement("div", { className: "nameNormal textRow ellipsis", style: { fontWeight: "600" } }, activity.details), BdApi.React.createElement("div", { className: "state textRow ellipsis" }, activity.state), activity?.timestamps?.end ? BdApi.React.createElement("div", { className: "mediaProgressBarContainer" }, BdApi.React.createElement(MediaProgressBar, { start: activity?.timestamps?.start, end: activity?.timestamps?.end })) : BdApi.React.createElement(ActivityTimer, { activity })), BdApi.React.createElement("div", { className: "buttonsWrapper actionsProfile" }, BdApi.React.createElement(SpotifyButtons, { user, activity }))))));
}
...
function StreamCards({ user, voice }) {
	...
	), BdApi.React.createElement("div", { className: "contentImagesProfile content" }, BdApi.React.createElement("h3", { className: "textRow", style: { display: "flex", alignItems: "center" } }, VoiceIcon({ channel }), BdApi.React.createElement("h3", { className: "nameWrap nameNormal textRow", style: { fontWeight: "600" } }, channel.name)), BdApi.React.createElement(
		VoiceList,
		{
			className: "userList",
...
function ConnectionComponent({ connectedAccount, userId }) {
	...
	return BdApi.React.createElement(ConnectionRenderer, { className: "connectedAccount", connectedAccount, userId });
}
...
function BioBuilder({ displayProfile }) {
	if (displayProfile?._guildMemberProfile?.bio && betterdiscord.Data.load("serverBio")) {
		return [
			BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["NepzEx"])),
...
function RoleBuilder({ user, data }) {
	...
	return [
		serverMember?.roles?.length !== 1 ? BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["LPJmLy"])) : BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["IqVT2N"])),
...
function FavoriteWidgetBuilder({ widget, game }) {
	...
	return BdApi.React.createElement("div", { className: "widgetCard", ref }, BdApi.React.createElement(TooltipBuilder, { note: game?.name }, loading ? BdApi.React.createElement(FallbackCover, { game }) : BdApi.React.createElement("div", { className: "gameCover" }, BdApi.React.createElement(
		"img",
		{
			alt: game?.name,
			className: "gameCover",
...
function ShelfWidgetBuilder({ game }) {
	...
	return BdApi.React.createElement("div", { style: { position: "relative" } }, BdApi.React.createElement(TooltipBuilder, { note: game?.name }, loading ? BdApi.React.createElement(FallbackCover, { game }) : BdApi.React.createElement("div", { className: "gameCover" }, BdApi.React.createElement(
		"img",
		{
			alt: game?.name,
			className: "gameCover",
...
function CurrentWidgetBuilder({ widget, game, index }) {
	...
	))), BdApi.React.createElement("div", { className: "widgetDetails" }, BdApi.React.createElement("h3", { className: "widgetTitle" }, game?.name || "Unknown Game"), widget.games[index].tags && BdApi.React.createElement(TagRenderer.Z, { tags: widget.games[index].tags, widgetType: widget.type, className: "tagListContainer" })));
}
...
function HeaderInnerBuilder({ user, currentUser, displayProfile, tagName, displayName }) {
	return BdApi.React.createElement("header", { className: "header" }, BdApi.React.createElement(AvatarFetch, { className: "avatar", user }), BdApi.React.createElement("div", { className: "headerInfo" }, BdApi.React.createElement("div", { className: "nameSection" }, BdApi.React.createElement("div", { className: "displayName" }, displayName || tagName), !betterdiscord.Data.load("disableDiscrim") && displayProfile._userProfile?.legacyUsername ? BdApi.React.createElement(
		"div",
		{
			className: "nameTag",
...
function AboutTab({ data, user, displayProfile }) {
	...
	return BdApi.React.createElement("div", { className: "infoScroller scrollerBase", style: { overflow: "hidden scroll", paddingRight: "12px" } }, displayProfile?.pronouns && BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["+T3RIy"])), BdApi.React.createElement("div", { className: "userPronouns", style: { color: "var(--text-default)", fontSize: "14px" } }, displayProfile.pronouns)), BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement(BioBuilder, { displayProfile })), BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement(RoleBuilder, { user, data, displayProfile })), BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["a6XYDw"])), BdApi.React.createElement("div", { className: "memberSinceWrapper", style: { display: "flex", gap: "8px", alignItems: "center" } }, MemberDateBuilder({ data, user }))), BdApi.React.createElement("div", { className: "userInfoSection" }, BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["PbMNh4"])), BdApi.React.createElement(NoteComponent, { userId: user.id })), betterdiscord.Data.load("boardTab") && user.id === data.currentUser.id && BdApi.React.createElement("div", { className: "userInfoSection", style: { paddingBottom: "20px" } }, BdApi.React.createElement("div", { className: "userInfoSectionHeader" }, intl.intl.formatToPlainString(intl.t["Jzj9q6"])), BdApi.React.createElement(
		"button",
		{
			className: `${ButtonClasses.button} ${ButtonClasses.sm} ${ButtonClasses.primary} ${ButtonClasses.hasText}`,
			onClick: () => ModalSystem.openModal(
				(props) => BdApi.React.createElement(ModalRoot.Modal, { ...props, title: intl.intl.formatToPlainString(intl.t["Jzj9q6"]) }, BdApi.React.createElement(Board, { user }))
			)
		},
		BdApi.React.createElement("div", { className: `${ButtonClasses.buttonChildrenWrapper}` }, BdApi.React.createElement("div", { className: `${ButtonClasses.buttonChildren}`, style: { fontSize: "14px" } }, intl.intl.formatToPlainString(intl.t["Geikws"])))
...
const styles = Object.assign(
	...
	betterdiscord.Webpack.getModule((x) => x.container && x.badge && !x.settingsTabBar && !x.guildPrefixContainer && !x.tabBar && !x.popout),
